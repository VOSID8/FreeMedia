...
trait WebCrawler {
  def run(
    seeds: Set[URLL],
    shouldCrawl: URLL => Boolean,
    processResult: (URLL, String) => UIO[Any]
...
object WebCrawler {
  ...
  def run(
    seeds: Set[URLL],
    shouldCrawl: URLL => Boolean,
    processResult: (URLL, String) => UIO[Unit]
  ...
  final case class WebCrawlerLive(web: Web) extends WebCrawler {
    ...
    def run(
      ...
      def modifyCrawlState(ref: Ref[CrawlState], urls: Set[URLL]): UIO[Set[URLL]] =
        ref.modify { crawlState =>
          (urls -- crawlState.visited, crawlState.visitAll(urls))
        }

      for {
        queue <- Queue.unbounded[URLL]
  ...
  def extractURLs(root: URLL, html: String): Set[URLL] = {
    val pattern = "href=[\"\']([^\"\']+)[\"\']".r
    ...
    scala.util.Try {
      ...
      for {
        m   <- matches
        url <- URLL.make(m) ++ root.relative(m)
      } yield url
    }
      .getOrElse(Set.empty)
  }
